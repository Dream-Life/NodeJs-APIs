...
// 如果 value 不是一个字符串， 数字， 或者 Buffer， 
// 该方法会抛出一个 TypeError 异常， 如果 value 是一个数字，
//  它将会被强制转换成一个有效的 byte 值， 该值介于0到255之间。

// 如果 byteOffset 不是一个数字， 它将会被强制转换成一个数字。
//  任何对 NaN 或者 0, 像 {}, [], null or undefined， 的参数，
//  将会搜索整个 buffer。 该行为和 String#indexOf() 保持一致。
...
// 如果 value 是一个空字符串或空 Buffer，
// 并且 byteOffset 小于 buf.length，返回 byteOffset。
// 如果 value 是一个空字符串，并且 byteOffset 大于 buf.length，返回 buf.length。